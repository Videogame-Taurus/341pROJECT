...
if (!isset($_SESSION['username'])) {
    ...
    header("Location: ../views/loginSelect.html");
    exit();
}
...
if ($conn->connect_error) {
    ...
    if (isset($_POST['submitPreferences'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO org_pref (Username, Interest, Skill) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $username, $interests1, $skills1);
        $stmt->execute();

        if ($stmt->affected_rows > 0) {
            $_SESSION['status'] = "Organization preferences updated successfully!";
            header("Location: ../views/organization-dashboard.php");
            exit();
        } else {
            $_SESSION['status'] = "Error updating organization preferences.";
            header("Location: ../views/organization-dashboard.php");
            exit();
        }
